...
- 📖 [Wikipedia — Sherman–Morrison formula](https://en.wikipedia.org/wiki/Sherman%E2%80%93Morrison_formula)
  ...
  form. Read after the lesson as a compact reference.
- 📖 [The Matrix Cookbook (Petersen & Pedersen), §3.2](https://www.math.uwaterloo.ca/~hwolkowi/matrixcookbook.pdf)
  ...
  a handy one-page lookup. Skim after the lesson when you want to see the full family of
  inverse identities in one place.
- 📖 [VMLS Ch. 11 — Matrix inverses](https://web.stanford.edu/~boyd/vmls/vmls.pdf)
  (book, PDF, pp. 207--224) — background on what a matrix inverse is and the cost of
  computing one, the footing this module's $O(p^3)$-vs-$O(p^2)$ argument stands on. Read
  before or alongside the lesson if "inverse" still feels fuzzy.

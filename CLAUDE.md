...
│   │   └── knowledge-extractor.md     # subagent: one-shot PDF → knowledge base extraction
...
│   │                                  #   source-licensing, knowledge-base + citations
│   │                                  #   contracts, and more — see the directory)
...
│   ├── gen-kb-index.R         # regenerates the knowledge blocks of _quarto.yml + knowledge/README.md
│   │                          #   from the knowledge/ pages themselves (--check = CI drift guard)
│   ├── check-indexes.R        # index-check CI: foundation↔course links, Used-by, Builds-on (+lesson sync),
│   │                          #   Status, page registration, KB citations/links (checks 8-10)
...
├── docs/                      # decision records (e.g., webr-decision.md) + gap-reports/ (knowledge-gap-check output)
├── knowledge/                 # the citable knowledge base (searchable encyclopedia on the site)
│   ├── README.md              # index: concepts + sources (table bodies GENERATED by gen-kb-index.R)
│   ├── references.bib         # THE one bibliography (citations contract: .claude/course-authoring/citations.md)
│   ├── glossary.md            # atomic terms, one cited row each (created on first use)
│   ├── concepts/<slug>.md     # one page per lesson-anchoring concept, every claim cited
│   └── sources/<slug>.md      # one record per ingested PDF: license verdict, big ideas,
│                              #   chapter coverage map (= the resumable extraction state)
...
- **Registration:** no per-course `_quarto.yml`. Two paths:
  ...
  - **Knowledge pages:** create/edit pages under `knowledge/` and run
    `Rscript scripts/gen-kb-index.R` — it regenerates the knowledge sentinel blocks
    in `_quarto.yml` and `knowledge/README.md` from the pages themselves (no
    meta.dcf). Same never-hand-edit rule; `gen-kb-index.R --check` guards it in CI.

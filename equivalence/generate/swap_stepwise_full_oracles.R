...
# best-subset search. The "real reference" for the Swap-Stepwise method is the full-data
...
GEN <- "equivalence/generate/swap_stepwise_full_oracles.R"
...
wr(list(
  meta = list(slug = "swap-stepwise-joint-from-summaries", tier = "full",
  ...
  "equivalence/fixtures/swap-stepwise-joint-from-summaries.json")

## 2) Switching search vs exhaustive BIC(lm) — clean case, true set {1,3,5} (seed 2)
wr(list(
  meta = list(slug = "swap-stepwise-selection-clean", tier = "full",
  ...
  "equivalence/fixtures/swap-stepwise-selection-clean.json")
...
wr(list(
  meta = list(slug = "swap-stepwise-selection-collinear", tier = "full",
              source = "exhaustive best-subset over BIC(lm), 2^12 subsets",
  ...
  "equivalence/fixtures/swap-stepwise-selection-collinear.json")
...
wr(list(
  meta = list(slug = "swap-stepwise-rank1-bordering", tier = "full",
  ...
  "equivalence/fixtures/swap-stepwise-rank1-bordering.json")
...
wr(list(
  meta = list(slug = "swap-stepwise-ridge", tier = "full",
  ...
  "equivalence/fixtures/swap-stepwise-ridge.json")

cat("\nAll Swap-Stepwise fixtures written. Verify with:  Rscript equivalence/check.R\n")

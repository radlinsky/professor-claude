...
**File:** source-papers/2021-james-introduction-statistical-learning.pdf
...
**Extraction goal:** Extract chapters 2, 4 (including §4.4 generative classifiers), 5, 6, 8, 12, 13 — targeted to feed foundations on bias-variance/model validation, classification evaluation, generative classifiers (LDA/QDA/NB), kNN, resampling/bootstrap, model selection/regularization, decision trees, bagging/random forests/boosting, PCA/clustering, and false discovery rate
...
| 4 — Classification | pp. 129–195 | done | classification-evaluation (created), lda-qda-and-naive-bayes (created from §4.4--4.5); glossary: AUC, confusion matrix, confounding, generalized linear model, linear discriminant analysis, link function, logistic regression, naive Bayes classifier, positive predictive value, posterior probability (classification), prior probability (classification), quadratic discriminant analysis, ROC curve, sensitivity, specificity | pp. 171--195 are R lab + exercises (skipped -- no new conceptual content) |
...
| 8 — Tree-Based Methods | pp. 327–365 | done | decision-trees (created from §8.1), bagging-random-forests-and-boosting (created from §8.2); glossary: bagging, boosting, cost complexity pruning, decision tree, Gini index, out-of-bag (OOB) error, random forest, recursive binary splitting, variable importance | pp. 353--365 are R lab + exercises (skipped -- no new conceptual content) |
...
- 2026-07-12 — Goal widened to add §4.4 (generative classifiers) and ch. 8 (trees and ensembles), to feed three new statistical-learning foundation modules: lda-qda-and-naive-bayes, decision-trees, bagging-random-forests-and-boosting.
- 2026-07-12 — Ch. 4 augmented (§4.4--4.5). Created concept page: lda-qda-and-naive-bayes. Covers generative classification via Bayes' theorem, LDA (shared covariance → linear boundary), QDA (per-class covariance → quadratic boundary), naive Bayes (independence assumption), and §4.5 method comparison. No new glossary entries needed (LDA, QDA, naive Bayes terms existed from prior extraction round). pp. 141--170 read.
- 2026-07-12 — Ch. 8 done (§8.1--8.2). Created concept pages: decision-trees (recursive binary splitting, RSS/Gini/entropy criteria, pruning, trees vs linear models, instability), bagging-random-forests-and-boosting (bootstrap aggregation, OOB error, random forests' de-correlation trick, variable importance, boosting, BART). Added 9 glossary entries. pp. 327--352 read; pp. 353--365 (R lab + exercises) skipped.

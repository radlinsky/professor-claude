...
| $\lVert\beta\rVert_2$ | "ell-two norm of beta" | $\sqrt{\sum_{j=1}^{p} \beta_j^2}$; the Euclidean norm of the coefficient vector [@james2021introductionstatisticallearning, p. 239] |
| $\lVert\beta\rVert_1$ | "ell-one norm of beta" | $\sum_{j=1}^{p} \lvert\beta_j\rvert$; used in the lasso penalty [@james2021introductionstatisticallearning, p. 242] |
| $s$ | "s" | The budget in the constraint form: $\sum \lvert\beta_j\rvert \leq s$ (lasso) or $\sum \beta_j^2 \leq s$ (ridge) [@james2021introductionstatisticallearning, pp. 243--244] |
...
- **The lasso produces sparse models; ridge does not** -- The $\ell_1$ penalty forces some coefficients to be exactly zero. In the simple orthonormal case ($n = p$, $\mathbf{X}^\top\mathbf{X} = \mathbf{I}$), ridge shrinks each coefficient proportionally: $\hat\beta^R_j = y_j / (1 + \lambda)$, while the lasso applies soft-thresholding: $\hat\beta^L_j = \text{sign}(y_j)(|y_j| - \lambda/2)_+$, setting coefficients to exactly zero when $|y_j| \leq \lambda/2$. [@james2021introductionstatisticallearning, pp. 247--248, Eqs. 6.14--6.15]
...
- **Bayesian interpretation** -- Ridge regression corresponds to the posterior mode of $\beta$ under a Gaussian prior with mean zero and standard deviation a function of $\lambda$. The lasso corresponds to the posterior mode under a double-exponential (Laplace) prior. The Laplace prior is steeply peaked at zero, concentrating more prior density near zero than the Gaussian; the MAP objective's $\ell_1$ penalty inherits the non-differentiability of $|\beta_j|$ at zero, which is what drives coefficients to exactly zero in the optimization. The Laplace prior itself is continuous and assigns zero probability to any single point (including zero). The Gaussian prior is flatter and fatter, encoding the belief that coefficients are distributed around zero. [@james2021introductionstatisticallearning, pp. 249--250]
...
- **MAP derivation of LASSO** -- Replacing the Gaussian prior with a Laplace prior $f_\Theta(\boldsymbol{\theta}) \propto \exp\{-\lVert\boldsymbol{\theta}\rVert_1/b\}$ (where $b$ is the Laplace scale parameter, and the density includes the normalizing constant $1/(2b)$ per coordinate) gives $\operatorname*{argmin}_{\boldsymbol{\theta}} (1/(2\sigma^2))\lVert\boldsymbol{y} - \boldsymbol{X}\boldsymbol{\theta}\rVert^2 + (1/b)\lVert\boldsymbol{\theta}\rVert_1$, i.e., LASSO with $\lambda = 2\sigma^2/b$ (the factor of 2 arises from the Laplace normalizing constant $1/(2b)$). The Laplace prior concentrates more density near zero than the Gaussian; sparsity in the LASSO solution comes from the non-differentiability of the $\ell_1$ penalty at zero in the MAP objective. [@chan2021probabilitydatascience, pp. 519--520, Eq. 8.54]
...
- **The simple special case ($n = p$, orthonormal X: $\mathbf{X}^\top\mathbf{X} = \mathbf{I}$)** -- ISL derives closed-form solutions for ridge ($\hat\beta^R_j = y_j/(1+\lambda)$, proportional shrinkage) and lasso ($\hat\beta^L_j$, soft-thresholding) in this idealized setting. The resulting Figure 6.10 shows two fundamentally different shrinkage behaviors: one line through the origin for ridge, a shifted and truncated line for lasso. This makes the general behavior concrete. [@james2021introductionstatisticallearning, pp. 247--248]
...
- **Gaussian vs. Laplace prior plots (Figure 6.11)** -- ISL displays the Gaussian (ridge) and double-exponential/Laplace (lasso) prior densities side by side. The Laplace prior concentrates more density near zero than the Gaussian, but as a continuous distribution it assigns zero probability to any single point (including zero). Sparsity in the lasso solution comes from the non-differentiability of the $\ell_1$ penalty at zero in the MAP objective, not from the prior placing positive probability mass at zero. The Gaussian is flatter and fatter, putting more prior density on large coefficient values. This Bayesian framing gives an additional lens on why the lasso produces sparse solutions. [@james2021introductionstatisticallearning, pp. 249--250]

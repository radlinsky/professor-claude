...
- **Beta prior for Bernoulli: MAP solution** -- With likelihood $\prod \theta^{x_n}(1-\theta)^{1-x_n}$ and prior $\text{Beta}(\alpha, \beta) \propto \theta^{\alpha-1}(1-\theta)^{\beta-1}$, the MAP estimate is $\hat{\theta}_{\text{MAP}} = (S + \alpha - 1)/(N + \alpha + \beta - 2)$, valid when $\alpha + S > 1$ and $\beta + N - S > 1$ (otherwise the posterior mode lies at a boundary: $\hat{\theta}_{\text{MAP}} = 0$ when $\alpha + S \leq 1$, or $\hat{\theta}_{\text{MAP}} = 1$ when $\beta + N - S \leq 1$). The hyperparameters $\alpha, \beta$ control the prior's shape: large $\alpha$ biases toward $\theta = 1$, large $\beta$ toward $\theta = 0$. [@chan2021probabilitydatascience, pp. 514--515, Eq. 8.49]

...
- **Singular values as max/min of $\lVert A\mathbf{u}\rVert$.** $\sigma_1 = \max_{\lVert\mathbf{u}\rVert=1} \lVert A\mathbf{u}\rVert$ is the maximum stretching. $\min_{\lVert\mathbf{u}\rVert=1} \lVert A\mathbf{u}\rVert$ equals $\sigma_n$ (or $\sigma_r$) when $A$ has trivial null space; when $A$ has a nontrivial null space (i.e., $\text{rank}(A) < n$), the minimum is 0 (achieved by any unit vector in $\text{Nul}(A)$). [@austin2022understandinglinearalgebra, pp. 459--460]

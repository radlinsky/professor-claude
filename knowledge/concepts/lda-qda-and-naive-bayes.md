...
**Bayes' theorem for classification** provides the foundation for all three methods. Given $K$ classes, the posterior probability that an observation with predictor vector $x$ belongs to class $k$ is $p_k(x) = \Pr(Y = k \mid X = x) = \pi_k f_k(x) / \sum_{l=1}^{K} \pi_l f_l(x)$, where $\pi_k = \Pr(Y = k)$ is the prior probability and $f_k(x)$ is the class-conditional density of the predictors given class $k$ (written $f_{X \mid Y}(x \mid k)$; for continuous predictors, this is a density, not a probability). The classifier assigns to the class with the highest $p_k(x)$. [@james2021introductionstatisticallearning, pp. 141--142, Eq. 4.15]
...
- **LDA is a special case of QDA** -- LDA is the special case of QDA where $\boldsymbol{\Sigma}_1 = \cdots = \boldsymbol{\Sigma}_K$. LDA is also a special case of Gaussian naive Bayes only when the shared covariance matrix $\boldsymbol{\Sigma}$ is diagonal (so the predictors are uncorrelated within each class). General LDA uses a full (non-diagonal) shared covariance matrix, while Gaussian naive Bayes restricts to diagonal covariance matrices (one per class). [@james2021introductionstatisticallearning, pp. 164--165]
...
- **The bias-variance trade-off governs the choice among methods** -- LDA estimates $K$ mean vectors ($Kp$ parameters) plus one shared covariance matrix ($p(p+1)/2$ unique entries); QDA estimates $K$ mean vectors plus $K$ separate covariance matrices ($K \cdot p(p+1)/2$ covariance parameters); Gaussian naive Bayes estimates $K$ mean vectors plus $K$ diagonal covariance matrices ($Kp$ variance parameters). With $n$ training observations: LDA can have high bias (if the common-covariance assumption is wrong) but low variance; QDA can have low bias but high variance when $n$ is small relative to $p$; naive Bayes has different bias (independence assumption may be wrong) but very low variance. [@james2021introductionstatisticallearning, pp. 153--154, 156--158]

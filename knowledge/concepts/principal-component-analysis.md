...
- **"PCA fails when data are not orthogonal."** PCA assumes the data have a decomposable structure along orthogonal axes. If the data live in a non-orthogonal manifold (e.g., a donut shape), PCA cannot find a low-dimensional approximation with $p \ll d$. Kernel PCA applies a nonlinear transformation before running standard PCA. [@chan2021probabilitydatascience, p. 311]

...
The confidence interval is a random interval (it changes with each new dataset); it is *not* the deterministic interval $[\theta - \epsilon, \theta + \epsilon]$. The randomness comes from $\hat{\Theta}$, not from $\theta$. The $1 - \alpha$ probability refers to the *procedure*: if we construct confidence intervals from many independent datasets, approximately $100(1 - \alpha)\%$ of them will contain $\theta$. A particular realized interval either contains $\theta$ or it does not -- it is not meaningful to assign a probability to that event in the frequentist framework. [@chan2021probabilitydatascience, pp. 547--549]

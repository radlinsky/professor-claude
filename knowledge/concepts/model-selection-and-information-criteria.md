...
- **Forward stepwise is not guaranteed to find the global best model** -- With $p = 3$ predictors, if the best one-variable model contains $X_1$ and the best two-variable model contains $X_2$ and $X_3$, forward stepwise will miss it because $\mathcal{M}_2$ must include $X_1$. However, in practice forward stepwise tends to perform well and is the only subset-selection method applicable when $p \geq n$ (regularization methods such as ridge regression, the lasso, and PCR can also handle $p \geq n$). [@james2021introductionstatisticallearning, pp. 230--231]

...
- **Complex eigenvalues.** A $2 \times 2$ matrix with complex eigenvalues $a \pm bi$ is similar to a scaled rotation block $r\begin{pmatrix}\cos\theta & -\sin\theta\\ \sin\theta & \cos\theta\end{pmatrix}$, where $r = \sqrt{a^2 + b^2}$ and $\theta = \operatorname{atan2}(b, a)$. (Using $\operatorname{atan2}$ instead of $\arctan(b/a)$ preserves quadrant information.) The matrix need not perform a Euclidean rotation in the standard basis, but it is similar to such a rotation-scaling. [@austin2022understandinglinearalgebra, p. 264]

...
When a statistics paper writes "the spectral decomposition of $\Sigma$" it means $\Sigma = Q\Lambda Q^T$ where $\Lambda = \text{diag}(\lambda_1, \ldots, \lambda_m)$ and $Q$ is orthogonal. In the multivariate normal density, the covariance matrix $\Sigma$ appears in $|\Sigma|^{-1/2} \exp(-\frac{1}{2}(\mathbf{x} - \boldsymbol{\mu})^T \Sigma^{-1} (\mathbf{x} - \boldsymbol{\mu}))$; the Spectral Theorem guarantees that this quadratic form can always be diagonalized. [@austin2022understandinglinearalgebra, pp. 419--420, 426]

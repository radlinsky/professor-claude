...
- **Overdetermined system: full rank** -- When $\boldsymbol{X} \in \mathbb{R}^{N \times d}$ with $N > d$ and $\text{rank}(\boldsymbol{X}) = d$ (full column rank), $\boldsymbol{X}^\top\!\boldsymbol{X}$ is invertible, and the least squares problem has the unique minimizer $\hat{\boldsymbol{\theta}} = (\boldsymbol{X}^\top\!\boldsymbol{X})^{-1}\boldsymbol{X}^\top\!\boldsymbol{y}$. If $\boldsymbol{y}$ lies in the column space of $\boldsymbol{X}$, then $\hat{\boldsymbol{\theta}}$ also satisfies $\boldsymbol{y} = \boldsymbol{X}\hat{\boldsymbol{\theta}}$ exactly; otherwise the system $\boldsymbol{y} = \boldsymbol{X}\boldsymbol{\theta}$ has no exact solution, and $\hat{\boldsymbol{\theta}}$ is the best approximation in the least-squares sense. [@chan2021probabilitydatascience, pp. 409--410, Thm 7.1]
